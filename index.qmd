...
* Code: versioning with Git, improving quality with formatters (`Ruff` !), community standard structure
* Configuration: virtual environments and consistent depency management (uv!), Docker for more control on external dependencies (more later)
* Data: S3 cloud storage with appropriate format (e.g. Parquet), pipeline approach (e.g. dbt)

![](excalidraw/projet_structure2.png)
...
* Is it possible to leapfrog when having missed the ML era ? 
...
> The gap between a notebook that works and a system that delivers value is not only a technical gap: it is an [**operational**]{.blue2} one.
