# Comment moduler la présentation {.backgroundTitre}

## Modularité de fichiers quarto (.qmd) {.backgroundStandard}

:::{.enteteAvecTitre}
Comment moduler la présentation ?
:::

:::{.listeSansPuceNiveau1}
- [Il est possible d'externaliser une partie du support en créant un fichier quarto et en l'intégrant dans le fichier principal avec le code suivant :]{.puceListFleche}
:::

[`{{< include _modulariteSupport.qmd >}}`]{.center}

:::{.listeSansPuceNiveau1}

- [Ce "shortcode" doit apparaitre seul sur une ligne et doit être entouré de lignes vides.]{.puceListFleche}
- [Cela revient à copier et coller le texte du fichier inclus dans le fichier principal.]{.puceListFleche}
- [Pour que chaque partie intègre le style de la page principale, il faut ajouter ce fichier dans la partie **render** du fichier *_quarto.yml*
]{.puceListFleche}
- Ce fichier doit être préfixé par un ["_"]{.rouge_insee} pour ne pas être ignoré par l'option render du fichier yml. 

:::

{{< include data/_data.qmd >}}

## Utilisation de variables {.backgroundStandard}

:::{.enteteAvecTitre}
Comment moduler la présentation ?
:::

#### Utilisation du fichier _variables.yml

:::{.listeSansPuceNiveau1}

- [il est possible de récupérer et utiliser les variables présentes dans ce fichier avec le shortcode [var]{.rouge_insee}:]{.puceListFleche}
[{{{< var version >}}} : {{< var version >}}]{.center}
:::

[[*Exemple :*]{.souligneRouge} Ainsi, au sein de l'{{< var organisation >}}, vous pouvez contacter le support technique via l'adresse mail suivante : {{< var supportTechnique >}}]{.tiny}

. . .


#### Utilisation des métadonnées

:::{.listeSansPuceNiveau1}

- [Pour utiliser un mot clé présent dans les parties yml des fichiers (ex: *_quarto.yml*), il faudra utiliser le shortcode [meta]{.rouge_insee} :]{.puceListFleche}
:::

[[*Exemple :*]{.souligneRouge} Pour ce support, les dimensions de la page sont de {{< meta height >}} (hauteur) x {{< meta width >}} (largeur). Ce support a pour vocation de créer un {{< meta description >}}.]{.tiny}

## Intégration de code R dans le support{.backgroundStandard}

:::{.enteteAvecTitre}
Comment moduler la présentation ?
:::

@fig-airquality further explores the impact of temperature on ozone level.

```{r}
#| echo: true
#| label: fig-airquality
#| fig-cap: Temperature and ozone level.
#| warning: false

library(ggplot2)

ggplot(airquality, aes(Temp, Ozone)) + 
  geom_point() + 
  geom_smooth(method = "loess")
```


## Intégration de code R dans le support (part2) {#ScriptR .backgroundStandard}

:::{.enteteAvecTitre}
Comment moduler la présentation ?
:::

#### Exemple 1 : avec un chunk

```{r}
#| layout: [1,1]
repartition_sex_genre

head(moy_taille_poids_selon_espece)
```

#### Exemple 2 : avec un chunk dans le chunk

````
---
title: "My document"
---

Some markdown content.

```{{r}}
1 + 1
```

Some additional markdown content.

````
## Intégration de code R dans le support (part3) {.backgroundStandard}

:::{.enteteAvecTitre}
Comment moduler la présentation ?
:::

```{r}

DT::datatable(moy_taille_poids_selon_sex)

```

